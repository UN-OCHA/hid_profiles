...
function sendReminderCheckinEmails(cb) {
  ...
  var stream = Contact.find({'type': 'local', $or: [{ 'remindedCheckin': null}, {'remindedCheckin': false}], 'status': true }).stream();
      ...
      if (!err && send) {
        ...
        mail.sendTemplate('reminder_checkin', mailOptions, function (err, info) {
          if (!err) {
            ...
            // set remindedCheckin to true
            contact.remindedCheckin = true;
            contact.remindedCheckinDate = current.toISOString();
            contact.save();
          }
        });
      }
