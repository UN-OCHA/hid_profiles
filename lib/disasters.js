...
async.auto({
  disasters: ['operations', cacheDisasters],
},
...
function newDisasterNotification(disaster) {
  ...
  async.series([
      ...
      if(disaster.countries.length > 0){
        async.forEachSeries(disaster.countries, function(country, callback2) {
          if (!requiredLocations[iso3Countries[country]]) {
            ...
            Contact.find({ $query: { type: 'local', location: iso3Countries[country]},  $orderby : {location: -1} }, function (err, contact){
              contact.forEach(function(contact){
                if(!contact){
                  console.log("No users found");
                  return cb();
                }   
                requiredLocations[iso3Countries[country]].contacts.push(contact);       
            });   
            callback2();         
          })
        }, cb);
    ...
    function (cb){
      ...
      mailOptions.subject = 'New Disaster in your region';
      mailOptions.contacts = false;

      if(disaster.countries.length > 0){
        async.forEachSeries(disaster.countries, function(country, cb3) {
          console.log("1");
          if(requiredLocations[iso3Countries[country]].contacts.length > 0)
            mailOptions.contacts = true;

          if (mailOptions.contacts === true) {
            
            console.log(requiredLocations[iso3Countries[country]].contacts.length);
            requiredLocations[iso3Countries[country]].contacts.forEach(function(contact){
