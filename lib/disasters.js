        ...
        if (err) {
          ...
          for(var i in disasters){
            if(!doc.data.hasOwnProperty(i))
              newDisasterNotification(disasters[i]);
          } 
      ...
      if(disaster.countries.length > 0){
        async.forEachSeries(disaster.countries, function(country, callback2) {
          if (!requiredLocations[iso3Countries[country]]) {
            ...
            Contact.find({ $query: { type: 'local', location: iso3Countries[country], status: true },  $orderby : {location: -1} }, function (err, contacts){
      ...
      if(disaster.countries.length > 0){
        async.forEachSeries(disaster.countries, function(country, cb3) { 
          async.forEachSeries(requiredLocations[iso3Countries[country]].contacts, function(contact, cb4) { 
            if (contact.email && contact.email[0] && contact.email[0].address) {
              mailOptions.subject = 'A disaster alert has occurred in ' + contact.location + ' - Update your profile';
